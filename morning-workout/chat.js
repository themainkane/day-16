...
conversation.forEach( (i, index) => {
 const robotSide = document.createElement('div')
 const unicornSide = document.createElement('div')
 setTimeout(() => {
 
  if (index % 2 !== 0) {
    robotSide.className = ('message message--local')
    conversationContainer.appendChild(robotSide)
    
    const robotSideText = document.createElement('div')
    robotSideText.className = ('message__text')
    robotSideText.innerHTML = i.text
    robotSide.appendChild(robotSideText)

  } else {
    unicornSide.className = ('message message--remote')
    conversationContainer.appendChild(unicornSide)
    
    const unicornSideText = document.createElement('div')
    unicornSideText.className = ('message__text')
    unicornSideText.innerHTML = i.text
    unicornSide.appendChild(unicornSideText)
  }
}, 1000 * index)
})